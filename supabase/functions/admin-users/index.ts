...
async function handleCreate(
  ...
  if (error) {
    if (
      ...
      // Each email may only belong to one workspace — reject instead of silently
      // linking an account that's already a member elsewhere.
      const { data: otherWorkspaceMembership } = await admin
        .from("workspace_members")
        .select("workspace_id")
        .eq("user_id", userId)
      ...
      if (otherWorkspaceMembership) {
        throw new Error(
          `${data.email} already has an account in another workspace. Each email can only belong to one workspace — use a different email for this member.`,
        );
      }
