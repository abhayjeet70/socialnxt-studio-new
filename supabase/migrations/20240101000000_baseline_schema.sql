-- SocialNxt baseline schema.
-- Squashed on 2026-08-06 from ~40 ad-hoc, unordered SQL files that had
-- accumulated in supabase/migrations/ and supabase/*.sql (some depending
-- on objects only defined in other loose files, making replay order
-- unsafe). This single migration creates the full schema as of that date
-- in dependency-safe order: tables, then sequences/triggers, then helper
-- functions, then storage buckets, then RLS. Schema changes after this
-- baseline are tracked as normal timestamped migrations below it.
...
-- Note: media_assets intentionally has no RLS (matches original migrations / live schema)
