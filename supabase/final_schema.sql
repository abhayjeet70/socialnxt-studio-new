...
-- This script contains all tables, columns, functions, triggers, and RLS policies from your migrations.
-- Redundant queries (like adding/dropping the same columns/policies) have been merged into their final state.
...
-- Note: media_assets did not have explicit RLS in original migrations
...
CREATE POLICY "Admins can delete posts" ON public.posts FOR DELETE USING ( public.is_workspace_admin(workspace_id) );
