...
import { usePosts, useCurrentWorkspace, useWorkspaceMembers, useIssues, useQuotations, toLinkEntries } from "@/lib/queries";
    ...
    const postEvents = filteredPosts
      .map((p) => {
        ...
        if (p.reference_content && Array.isArray(p.reference_content)) {
          toLinkEntries(p.reference_content).forEach((entry) => {
            const isLink = !entry.url.match(/\.(jpeg|jpg|png|gif|webp)/i) && !entry.url.includes("supabase.co");
            events.push({
              ts: new Date(p.updated_at),
              who,
              ...
              extra: isLink ? (entry.name ? `${entry.name} (${entry.url})` : entry.url) : "📎 media file",
            });
          });
        }
        ...
        if (p.completed_work && Array.isArray(p.completed_work)) {
          toLinkEntries(p.completed_work).forEach((entry) => {
            const isLink = !entry.url.match(/\.(jpeg|jpg|png|gif|webp)/i) && !entry.url.includes("supabase.co");
            events.push({
              ts: new Date(p.updated_at),
              who,
              ...
              extra: isLink ? (entry.name ? `${entry.name} (${entry.url})` : entry.url) : "📎 final file",
            });
          });
        }
