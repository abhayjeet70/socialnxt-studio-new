...
export function MediaPage() {
  ...
  const matchesBaseFilters = (a: MediaAsset) => {
    if (workspace?.role === "client") {
      ...
      const isTagged = (a.tagged_user_ids || []).includes(workspace.userId);
      if (!isAssigned && !isTagged && a.uploaded_by !== workspace.userId) return false;
    }
  ...
  // Team members that can be tagged on an asset (no clients).
  const taggableMembers = members.filter(m => m.role === "admin" || m.role === "employee");
  ...
  const toggleAssetTag = async (a: MediaAsset, userId: string, checked: boolean) => {
    const current = a.tagged_user_ids || [];
    const next = checked ? [...current, userId] : current.filter(id => id !== userId);
    try {
      await updateAsset.mutateAsync({ id: a.id, workspace_id: workspace!.workspaceId, updates: { tagged_user_ids: next } });
      toast.success(checked ? "Member tagged" : "Tag removed");
  ...
  const handleAddLink = async ({ name, url }: { name: string; url: string }) => {
    ...
    if (workspace.role !== "client" && selectedClient === "all") {
      toast.error("Please select a client to add this link for.");
      return;
    }
    const payload: any = {
      workspace_id: workspace.workspaceId,
    ...
    if (workspace.role === "client") {
      ...
      if (myClientId) payload.client_id = myClientId;
    } else if (selectedClient !== "all") {
      payload.client_id = selectedClient;
    }
    try {
        ...
        <div className="flex flex-col gap-3">
          ...
          {workspace?.role !== "client" && (
            <div className="flex flex-wrap items-center gap-2 p-2 bg-muted/30 rounded-xl border border-dashed">
              ...
              <Button variant="outline" onClick={() => { if (selectedClient === "all") toast.error("Please select a client to add this link for."); else setAddLinkOpen(true); }} disabled={!workspace} className="rounded-lg h-9 text-xs bg-white">
            ...
            >
              ...
              <div className="p-3 flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-1.5 pt-2 border-t">
                  ...
                  {workspace?.role !== "client" && taggableMembers.length > 0 && (
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button className="text-[10px] text-muted-foreground bg-transparent border-none p-0 h-auto focus:ring-0 cursor-pointer hover:text-foreground w-full mt-0.5 text-left outline-none flex flex-wrap items-center gap-1">
                          <User className="h-3 w-3 shrink-0" />
                          <span className="truncate">
                            {(a.tagged_user_ids || []).length > 0
                              ? (a.tagged_user_ids || []).map(id => memberLabel(id).split(" ")[0]).join(", ")
                              : "Tag team member"}
                          </span>
                        </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="start" className="w-64 max-h-60 overflow-y-auto">
                        {taggableMembers.map((m) => (
                          <DropdownMenuCheckboxItem
                            key={m.user_id}
                            checked={(a.tagged_user_ids || []).includes(m.user_id)}
                            onCheckedChange={(checked) => toggleAssetTag(a, m.user_id, !!checked)}
                          >
                            <span className="flex flex-col min-w-0 flex-1" title={memberLabel(m.user_id)}>
