...
// Linear forward flow. 'changes_requested' is a side-branch off pending_approval/approved,
// not part of the line — see STATUS_FLOW_INDEX below.
const STATUS_FLOW = ["draft", "pending_approval", "approved", "scheduled", "published"] as const;
...
const STATUS_STYLES: Record<string, { bg: string; fg: string }> = {
  ...
  changes_requested: { bg: "#fee2e2", fg: "#dc2626" },
...
export function TasksPage() {
  ...
  // "View all clients" toggle is authoritative — same scoping rule as the Client
  // Management page. Only restrict to assigned clients when the toggle is off.
  const clientNamesSet = new Set(
    clients
      .filter(c => c.status !== "Closed")
      .filter(c => hasPermission("view_clients") || (isSMM
        ? c.team_assignments?.["Account/Social Media Manager"] === workspace?.userId
        : Object.values(c.team_assignments || {}).includes(workspace?.userId!)))
      .map(c => c.name)
  );
  ...
  const processImport = async (mode: ImportMode) => {
    ...
    try {
      ...
      // importExcel.ts already produces reference_content/completed_work as {name,url}[];
      // append the just-uploaded image URLs (raw strings) in the same shape.
      const asLinkEntries = (urls: string[] | undefined): LinkEntry[] =>
        (urls || []).filter(Boolean).map((url) => ({ name: null, url }));
      ...
      const finalRows = rowsToInsert.map(row => {
        ...
        return {
          ...restRow,
          reference_content: [...(row.reference_content || []), ...asLinkEntries(entry?.ref)],
          completed_work: [...(row.completed_work || []), ...asLinkEntries(entry?.comp)],
        };
      });
  ...
  const handleExportCSV = () => {
    ...
    const csv = sortedPosts.map(p => {
      ...
      const reference = toLinkEntries(p.reference_content).map(e => e.name ? `${e.name} (${e.url})` : e.url).join('; ');
      const completed = toLinkEntries(p.completed_work).map(e => e.name ? `${e.name} (${e.url})` : e.url).join('; ');
...
function TaskRow({ post, index, isClient, allClientNames,
  ...
  const { hasPermission } = usePermissions();
  const isSMM = workspace?.role === "employee" && workspace?.agencyRole === "Social Media Manager";
  const postClient = clients.find(c => c.name === post.client_name);
  const { data: clientSocials = [] } = useClientSocials(postClient?.id);
  ...
  const [addLinkTarget, setAddLinkTarget] = useState<"reference_content" | "completed_work" | null>(null);
  ...
  const handleMediaUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Append to the existing array of entries
      const existing = toLinkEntries(post[uploadingTarget]);
      updatePost.mutate({
        id: post.id,
        updates: { [uploadingTarget]: [...existing, { name: null, url }] }
      }, {
  ...
  const handleAddLink = (target: "reference_content" | "completed_work") => {
    setAddLinkTarget(target);
  };

  const submitAddLink = (entry: LinkEntry) => {
    if (!addLinkTarget) return;
    const existing = toLinkEntries(post[addLinkTarget]);
    updatePost.mutate({
      id: post.id,
      updates: { [addLinkTarget]: [...existing, entry] }
    });
    setAddLinkTarget(null);
  };

  const handlePickDesign = (asset: DesignerAsset) => {
    if (!designPickerTarget || !workspace) return;
    const target = designPickerTarget;
    // Attaching flips the design from private to visible for admin/allocated
    // employees on this client — enforced by the designer_assets RLS policies.
    attachDesign.mutate({
      id: asset.id,
      postId: post.id,
      clientId: postClient?.id || null,
    ...
    return (
      <div className="flex flex-wrap gap-2 mb-2">
        {entries.map((entry, i) => {
          ...
          if (isImage) {
            return (
              <div key={i} className="relative group w-16 h-16 rounded overflow-hidden border border-border shrink-0">
                <button type="button" onClick={() => setLightboxImage(entry.url)} className="block w-full h-full cursor-zoom-in">
                  <img src={entry.url} alt={entry.name || "media"} className="w-full h-full object-cover hover:scale-105 transition-transform" />
                </button>
                <button
                  type="button"
          ...
          return <LinkChip key={i} entry={entry} index={i} onRemove={removeEntry} />;
        })}
      </div>
    );
  };
    ...
    <tr id={`post-row-${post.id}`} className={`border-b border-gray-200 ${rowColor} hover:bg-gray-50 transition-colors`}>
      ...
      <td className="p-0 border-r border-gray-200 align-top bg-transparent">
        <PlatformMultiSelect
          ...
          availablePlatforms={post.client_name
            ...
            : allWorkspacePlatforms}
        />
        {/* Clickable links to the client's actual account(s) for each selected platform */}
        {(post.platforms ?? (post.platform ? [post.platform] : [])).length > 0 && (
          <div className="flex flex-wrap gap-1.5 px-3 pb-2">
            {(post.platforms ?? (post.platform ? [post.platform] : [])).map((p) => {
              const accountsForPlatform = clientSocials.filter(a => a.platform === p);
              const chosenId = post.platform_accounts?.[p];
              const chosen = accountsForPlatform.find(a => a.id === chosenId) || accountsForPlatform.find(a => a.is_primary) || accountsForPlatform[0];
              return (
                <div key={p} className="flex items-center gap-1">
                  {chosen?.profile_url ? (
                    <a
                      href={chosen.profile_url}
                      target="_blank"
                      rel="noreferrer"
                      className="text-[10px] text-primary hover:underline flex items-center gap-1"
                      title={`Open ${p}${chosen.label ? ` — ${chosen.label}` : ""}`}
                    >
                      <PlatformLogo platform={p} size={10} /> {p}{chosen.label ? ` (${chosen.label})` : ""}
                    </a>
                  ) : (
                    <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                      <PlatformLogo platform={p} size={10} /> {p}
                    </span>
                  )}
                  {accountsForPlatform.length > 1 && !isClient && (
                    <select
                      value={chosen?.id || ""}
                      onChange={(e) => updatePost.mutate({
                        id: post.id,
                        updates: { platform_accounts: { ...(post.platform_accounts || {}), [p]: e.target.value } },
                      })}
                      className="text-[9px] border border-gray-200 rounded px-1 py-0.5 bg-white focus:outline-none"
                      title={`Which ${p} account is this post for?`}
                    >
                      {accountsForPlatform.map(a => (
                        <option key={a.id} value={a.id}>{a.label || a.handle || a.username || "Account"}</option>
                      ))}
                    </select>
                  )}
                </div>
        ...
        <div className="flex flex-wrap gap-2 print-hide">
          ...
          {isDesigner && (
            <Button variant="outline" size="sm" className="h-8 text-xs px-2.5 bg-white/50" onClick={() => setDesignPickerTarget("reference_content")}>
              <ImageIcon className="w-3.5 h-3.5 mr-1" /> From My Designs
            </Button>
          )}
        </div>
      </td>
      ...
      <td className="px-4 py-3 align-middle text-center">
        <div className="flex flex-col items-center gap-2">
          {(() => {
            const canSubmitDraft = workspace?.role === "employee" || workspace?.role === "admin";
            const canApprovePending = workspace?.role === "client" || workspace?.role === "admin" || isSMM;
            const canScheduleApproved = workspace?.role === "admin" || isSMM;
            const canPublish = workspace?.role === "admin" || hasPermission("mark_posted");
            const canRevert = workspace?.role === "admin" || workspace?.role === "employee";

            const effectiveIndex = post.status === "changes_requested"
              ? STATUS_FLOW.indexOf("pending_approval")
              : STATUS_FLOW.indexOf(post.status as (typeof STATUS_FLOW)[number]);

            const forwardOptions: string[] = [];
            if (post.status === "draft" && canSubmitDraft) forwardOptions.push("pending_approval");
            if (post.status === "pending_approval" && canApprovePending) forwardOptions.push("approved", "changes_requested");
            if (post.status === "changes_requested" && canSubmitDraft) forwardOptions.push("pending_approval");
            if (post.status === "approved" && canScheduleApproved) forwardOptions.push("scheduled");
            if ((post.status === "approved" || post.status === "scheduled") && canPublish) forwardOptions.push("published");

            const revertOptions = canRevert && effectiveIndex > 0 ? STATUS_FLOW.slice(0, effectiveIndex) : [];
            ...
            const handleStatusChange = (newStatus: string) => {
              ...
              // Revert — jump back to any earlier stage. Admin/employee only, never client.
          ...
          <AddLinkDialog
            ...
            onOpenChange={(open) => !open && setAddLinkTarget(null)}
            onSubmit={submitAddLink}
            title={addLinkTarget === "reference_content" ? "Add reference link" : "Add completed content link"}
          />
          {isDesigner && (
            <DesignPickerDialog
              open={designPickerTarget !== null}
              onOpenChange={(open) => !open && setDesignPickerTarget(null)}
              assets={myDesigns.filter(d => !d.attached_to_post_id)}
