...
import { PermissionOverrideDialog } from "@/components/permission-override-dialog";
...
import {
  ...
  useWorkspacePermissionOverrides,
} from "@/lib/queries";
...
export function SettingsPage() {
  ...
  const { data: permissionOverrides = [] } = useWorkspacePermissionOverrides(workspace?.workspaceId);
  const [overrideDialogPerm, setOverrideDialogPerm] = useState<{ key: string; label: string } | null>(null);
  const employeeOptions = members
    .filter((m) => m.role === "employee")
    .map((m) => ({ user_id: m.user_id, name: m.users?.full_name || m.users?.email?.split("@")[0] || "Unknown" }));
  ...
  return (
    <AppShell title="Settings" subtitle="Manage members, roles, departments and workspace preferences.">
          ...
          <div className="card-soft p-4 sm:p-5">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                {isAdmin && (
                  <Button className="h-10 rounded-xl" onClick={() => setIsAddMemberOpen(true)}>
                      ...
                      <div>
                        ...
                        <div className="space-y-0 border border-border rounded-xl overflow-hidden">
                          {DEFAULT_PERMISSIONS.map((p, idx) => {
                            ...
                            const overrideCount = role === "employee"
                              ? permissionOverrides.filter((o) => o.permissions?.[p.key] !== undefined).length
                              : 0;
                            return (
                              <div key={p.key} className={`flex items-center justify-between px-4 py-3 ${idx !== DEFAULT_PERMISSIONS.length - 1 ? "border-b border-border/60" : ""} ${idx % 2 === 0 ? "bg-background" : "bg-muted/20"}`}>
                                <div>
                                  ...
                                  <div className="text-[10px] text-muted-foreground mt-0.5">
                                    Applies to: {role === "employee" ? "All Employees" : role === "admin" ? "All Admins" : "All Clients"}
                                  </div>
                                </div>
                                <div className="flex items-center gap-2">
                                  {isAdmin && role === "employee" && (
                                    <Button
                                      ...
                                      onClick={() => setOverrideDialogPerm({ key: p.key, label: p.label })}
                                    >
                                      Specific employees{overrideCount > 0 ? ` (${overrideCount})` : ""}
                                    </Button>
                                  )}
                                  {isAdmin ? (
                                    <Switch
      ...
      <AddMemberDialog
        ...
        onCreated={() => queryClient.invalidateQueries({ queryKey: ["workspace_members"] })}
      />

      {overrideDialogPerm && (
        <PermissionOverrideDialog
          open={overrideDialogPerm !== null}
          onOpenChange={(open) => !open && setOverrideDialogPerm(null)}
          permKey={overrideDialogPerm.key}
          permLabel={overrideDialogPerm.label}
          workspaceId={workspace?.workspaceId}
