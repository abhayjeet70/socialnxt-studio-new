...
export function MeetingsPage() {
  ...
  // Admins see every meeting in the workspace. Everyone else (employees and
  // clients) only sees meetings they organised or were added as a participant to.
  const meetings = workspace?.role === "admin"
    ? allMeetings
    : allMeetings.filter((m) => {
        const pIds = (m as any).participant_ids || [];
        return (
          pIds.includes(workspace?.userId) ||
          (m as any).client_id === workspace?.userId ||
          m.created_by === workspace?.userId
        );
      });
