...
import { useCurrentWorkspace, usePosts, useUpdatePostStatus, Post, useClients, useWorkspaceMembers, useCreatePost, toLinkEntries, type LinkEntry } from "@/lib/queries";
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {selectedDayPosts.length === 0 ? (
                  ...
                  selectedDayPosts.map((post) => {
                    const color = getPlatformColor(post);
                    const text = getPostText(post);
                    const isImageUrl = (entry: LinkEntry) =>
                      !!entry.url.match(/\.(jpeg|jpg|gif|png|webp)/i) || entry.url.includes("supabase.co");
                    const allMedia = [...toLinkEntries(post.completed_work), ...toLinkEntries(post.reference_content)];
                    const previewUrl = allMedia.find(isImageUrl)?.url;
                            ...
                            <div className="mt-2 space-y-2 border-t border-border pt-2">
                              ...
                              {(post.completed_work?.length || post.reference_content?.length) ? (
                                <div className="flex flex-wrap gap-1.5">
                                  {[...toLinkEntries(post.completed_work), ...toLinkEntries(post.reference_content)].slice(0, 4).map((entry, i) => (
                                    <a
                                      key={i}
                                      href={entry.url}
                                      target="_blank"
