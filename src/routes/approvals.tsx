...
function CompletedWorkMedia({ urls, onZoom }: { urls: unknown; onZoom: (url: string) => void }) {
      ...
      {entries.map((entry, i) =>
        isImageUrl(entry.url) ? (
          <button
            key={i}
            type="button"
            onClick={() => onZoom(entry.url)}
            className="w-12 h-12 rounded overflow-hidden border border-border shrink-0 cursor-zoom-in"
            title="Click to preview"
          >
            <img src={entry.url} alt={entry.name || "reference"} className="w-full h-full object-cover hover:scale-105 transition-transform" />
          </button>
        ) : (
...
export function ApprovalsPage() {
  ...
  const handleApprove = (post: Post) => {
    ...
    updateStatus.mutate(
      ...
      {
        onSuccess: () => {
          toast.success(`Approved "${post.topic || "task"}" for ${post.client_name}`);
  ...
  return (
    ...
    >
      ...
      <ApproveConfirmDialog
        post={confirmingPost}
        open={confirmingPost !== null}
        onOpenChange={(open) => !open && setConfirmingPost(null)}
        onConfirm={() => confirmingPost && handleApprove(confirmingPost)}
