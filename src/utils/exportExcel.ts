...
export type ExportLinkEntry = { name: string | null; url: string };
...
export type ExportPost = {
  ...
  topic: string | null;
  reference_content: ExportLinkEntry[] | null;
  completed_work: ExportLinkEntry[] | null;
...
/** Tolerant of the legacy bare-string[] shape some rows may still carry. */
function asLinkEntries(value: ExportLinkEntry[] | string[] | null | undefined): ExportLinkEntry[] {
  if (!value) return [];
  return value.map((v) => (typeof v === "string" ? { name: null, url: v } : v));
}
...
export async function exportContentSheetToExcel(opts: ExportOptions): Promise<void> {
  ...
  for (let i = 0; i < posts.length; i++) {
    ...
    const refEntries  = asLinkEntries(p.reference_content);
    const compEntries = asLinkEntries(p.completed_work);
    const refImages   = refEntries.filter((e) => isImageUrl(e.url)).map((e) => e.url);
    const compImages  = compEntries.filter((e) => isImageUrl(e.url)).map((e) => e.url);
    const refLinks    = refEntries.filter((e) => !isImageUrl(e.url)).map((e) => (e.name ? `${e.name} (${e.url})` : e.url));
    const compLinks   = compEntries.filter((e) => !isImageUrl(e.url)).map((e) => (e.name ? `${e.name} (${e.url})` : e.url));
