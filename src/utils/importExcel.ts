...
export async function parseExcelForPreview(file: File, workspaceId: string, authorId: string, members: any[]): Promise<ImportPreviewData> {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    const finalRefLinks: { name: string | null; url: string }[] = [];
    if (refColIdx !== -1) {
      ...
      if (buffer) {
        ...
        finalRefLinks.push({ name: null, url: `LOCAL_IMG_ROW_${rowNumber}_REF` });
      } else if (refLinks && refLinks !== "No media added") {
        finalRefLinks.push(...refLinks.split(',').map(s => s.trim()).filter(Boolean).map(url => ({ name: null, url })));
      }
    }

    const finalCompLinks: { name: string | null; url: string }[] = [];
    if (compColIdx !== -1) {
      ...
      if (buffer) {
        ...
        finalCompLinks.push({ name: null, url: `LOCAL_IMG_ROW_${rowNumber}_COMP` });
      } else if (compLinks && compLinks !== "No media added") {
        finalCompLinks.push(...compLinks.split(',').map(s => s.trim()).filter(Boolean).map(url => ({ name: null, url })));
      }
    }
