...
import {
  ...
  Search, Bell, ChevronDown, LogOut, Menu, X, Activity, Archive, Receipt, Images, Palette,
} from "lucide-react";
...
function SidebarContent({ workspace, pathname, onNavClick }: {
  ...
  const visibleNav = NAV.filter(item => {
    ...
    if (item.to === "/designs") {
      return role === "employee" && workspace?.agencyRole === "Designer";
    }
    ...
    <div className="flex flex-col h-full bg-sidebar text-sidebar-foreground">
      ...
      <div className="border-t border-white/10 p-4">
        <button
          ...
          className="flex items-center gap-3 text-sm text-sidebar-muted hover:text-white"
        >
...
export function AppShell({ children, title, subtitle, actions }: {
  ...
  const recentActivities = [
    ...posts.slice(0, 5).map(p => ({
      ...
      who: p.users?.full_name || p.users?.email || "Someone",
      action: "added",
      subject: "Task " + (p.topic || "") + (p.client_name ? ` for ${p.client_name}` : ""),
